...
Orgs.forEach(org => {
    org.spec.apps?.forEach(app => {
        app.spec.components?.forEach(component => {
            ...
            // Create repo
            repoFileContent += `gh repo create ${org ? `${org}/` : "" }${repo} --private --description "Source code for ${app.spec.name}" --gitignore "Node" --license "MIT" || echo "Repo ${org ? `${org}/` : "" }${repo} exists"\n`;
        })
    })
})
